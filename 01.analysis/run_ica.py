...
arguments.add_argument(
    ...
    dest="load_merged",
    action="store_true",
    help=("Check if there is a previous merged timeseries and if so load it."),
    default=False,
)
arguments.add_argument(
    "-iced",
...
arguments.add_argument(
    ...
    dest="ncomp",
    type=int,
    help=("Number of components to extract. Optional. Default 15."),
    default=15,
)
arguments.add_argument(
...
if args.load_merged and os.path.isfile(
    os.path.join(outdir, f"{fileprefix}_merged.nii.gz")
):
    data = f.get_fdata()
else:
    data = {}
    ...
    for file_path in indir.iterdir():
        if file_path.is_file() and fileprefix in str(file_path):
    ...
    if len(runs) == 0 or (len(runs) == 1 and not runs[0]):
        ...
        out_img = nib.Nifti1Image(data, f.affine, tmp_header)
...
mask = (
    nib.load(mname).get_fdata()
    if mname is not None
    else np.squeeze(np.any(data, axis=-1))
)
data = data[mask != 0]
...
if args.load_ica and os.path.isfile(os.path.join(outdir, f"ICA_{ncomp}_voxs.nii.gz")):
    voxss = nib.load(os.path.join(outdir, f"ICA_{ncomp}_voxs.nii.gz")).get_fdata()[
        mask != 0
    ]
    ...
    ts = np.dot(np.dot(tdata, voxs), np.diag(1.0 / s))
...
# Compute frequencies
num_samples = tss.shape[0]
tr = f.header["pixdim"][4]
time = np.arange(num_samples) * tr


fft_magnitudes = np.abs(np.fft.rfft(tss, axis=0)) * 2 / num_samples

freqs = fft_magnitudes[: num_samples // 2, :]

frequencies = np.fft.fftfreq(num_samples, d=tr)[: num_samples // 2]
...
# Plot
width = 25
height = tss.shape[1] * (width / 16) * 0.9
fig, axes = plt.subplots(
    nrows=tss.shape[1], ncols=1, figsize=(width, height), sharex=True
)
